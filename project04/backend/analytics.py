...
from backend.models import (
    ...
    MaterialPrice, DisposalCost, SegregationAudit, RevenueEntry, CostEntry
)
...
class AnalyticsEngine:
    ...
    def _get_daily_nwv(self, company_id: str, start_date: datetime, end_date: datetime) -> List[Dict]:
        """Get daily NWV values using actual revenue and cost entries when available"""
        ...
        if not transactions:
            print(f"[Analytics] No transactions found, returning empty list")
            return []
        ...
        if transaction_ids:
            ...
            # Group by transaction_id and sum actual revenue
            for entry in entries:
                if entry.transaction_id not in revenue_entries:
                    revenue_entries[entry.transaction_id] = 0.0
                revenue_entries[entry.transaction_id] += entry.actual_revenue
        ...
        if transaction_ids:
            ...
            # Group by transaction_id and sum actual costs
            for entry in entries:
                if entry.transaction_id not in cost_entries:
                    cost_entries[entry.transaction_id] = 0.0
                cost_entries[entry.transaction_id] += entry.actual_cost
        
        print(f"[Analytics] Found {len(revenue_entries)} transactions with revenue entries")
        print(f"[Analytics] Found {len(cost_entries)} transactions with cost entries")
        
        # Group by date - use actual revenue/cost if available, otherwise use calculated
        daily_data = {}
        for txn in transactions:
            ...
            # Use actual revenue from RevenueEntry if available, otherwise use calculated
            actual_revenue = revenue_entries.get(txn.id, None)
            if actual_revenue is not None:
                daily_data[date_key]["revenue"] += actual_revenue
            else:
                daily_data[date_key]["revenue"] += (txn.total_revenue or 0.0)
            
            # Use actual cost from CostEntry if available, otherwise use calculated
            actual_cost = cost_entries.get(txn.id, None)
            if actual_cost is not None:
                daily_data[date_key]["cost"] += actual_cost
            else:
        ...
        if result:
            print(f"[Analytics] Sample data: {result[0]}")
        return result
    ...
    def forecast_nwv(
        ...
        # If less than 7 days, still provide forecast but with lower confidence
        if len(daily_nwv) < 7:
            # Use available data but mark as limited
        ...
        if use_limited_data:
            window = len(nwv_values)  # Use all available data
            confidence_adjustment = 0.3  # Reduce confidence for limited data
        else:
        ...
        if method == "moving_average":
            ...
            recent_avg = statistics.mean(nwv_values[-window:]) if nwv_values else 0.0
            
            forecast_nwv = recent_avg * forecast_days
            forecast_revenue = statistics.mean([d["revenue"] for d in daily_nwv[-window:]]) * forecast_days if daily_nwv else 0.0
            forecast_cost = statistics.mean([d["cost"] for d in daily_nwv[-window:]]) * forecast_days if daily_nwv else 0.0
            ...
            if len(nwv_values) >= 2:
                ...
                forecast_nwv = statistics.mean(nwv_values) * forecast_days if nwv_values else 0.0
                forecast_revenue = statistics.mean([d["revenue"] for d in daily_nwv]) * forecast_days if daily_nwv else 0.0
                forecast_cost = statistics.mean([d["cost"] for d in daily_nwv]) * forecast_days if daily_nwv else 0.0
        else:
        ...
        if len(nwv_values) > 1:
            std_dev = statistics.stdev(nwv_values)
            mean_val = statistics.mean(nwv_values)
            coefficient_of_variation = (std_dev / abs(mean_val)) if mean_val != 0 else 1.0
            base_confidence = max(0.5, min(0.95, 1.0 - coefficient_of_variation))
        else:
            base_confidence = 0.5
        
        # Adjust confidence for limited data
        confidence = max(0.3, base_confidence - confidence_adjustment)
        ...
        result = {
            ...
            "method": method,
            "data_points_used": len(daily_nwv),
            "is_limited_data": use_limited_data
        }
        
        # Only save forecast if we have reasonable confidence
        if confidence >= 0.3:  # Lower threshold to save even limited forecasts
            try:
                ...
                print(f"[Analytics] Warning: Could not save forecast record: {e}")
                # Continue anyway - forecast result is still valid
    ...
    def generate_cost_optimizations(self, company_id: str) -> List[Dict]:
        ...
        # Fetch actual revenue and costs from entries
        transaction_ids = [txn.id for txn in transactions]
        revenue_entries = {}
        cost_entries = {}
        ...
        if transaction_ids:
            # Get revenue entries
            rev_entries = self.db.query(RevenueEntry).filter(
                RevenueEntry.transaction_id.in_(transaction_ids)
            ).all()
            ...
            for entry in cost_entry_objects:
                if entry.transaction_id not in cost_entries:
                    cost_entries[entry.transaction_id] = 0.0
                cost_entries[entry.transaction_id] += entry.actual_cost
        
        # Helper function to get actual revenue for a transaction
        def get_actual_revenue(txn):
            return revenue_entries.get(txn.id, txn.total_revenue or 0.0)
        
        def get_actual_cost(txn):
            return cost_entries.get(txn.id, txn.disposal_cost or 0.0)
        
        # 1. Segregation Quality Optimization
        quality_scores = [t.quality_score for t in transactions if t.quality_score]
        if quality_scores:
            avg_quality = statistics.mean(quality_scores)
            if avg_quality < 0.85:
                potential_savings = self._calculate_segregation_savings(transactions, avg_quality, get_actual_revenue)
                if potential_savings > 0:
                    total_cost = sum(get_actual_cost(t) for t in transactions)
        ...
        volume_optimization = self._analyze_volume_pricing(transactions, company.location, get_actual_revenue)
        if volume_optimization:
            optimizations.append(volume_optimization)
        
        # 3. Material Mix Optimization
        material_mix = self._analyze_material_mix(transactions, get_actual_revenue)
        if material_mix:
    ...
    def _calculate_segregation_savings(self, transactions: List[WasteTransaction], current_avg: float, get_revenue_func) -> float:
        ...
        current_revenue = sum(get_revenue_func(t) for t in recyclable_txns)
    ...
    def _analyze_volume_pricing(self, transactions: List[WasteTransaction], location: str, get_revenue_func) -> Optional[Dict]:
        ...
        for material, total_volume in material_volumes.items():
            ...
            if volume_price:
                current_revenue = sum(
                    get_revenue_func(t) for t in transactions 
    ...
    def _analyze_material_mix(self, transactions: List[WasteTransaction], get_revenue_func) -> Optional[Dict]:
        ...
        for txn in transactions:
            if txn.quality_score and txn.quality_score < 0.7:
                ...
                low_quality_materials[txn.material_type]["total_revenue"] += get_revenue_func(txn)
        ...
        for material, data in low_quality_materials.items():
            ...
            potential_revenue = current_revenue * (0.85 / avg_quality) if avg_quality > 0 else current_revenue  # Improve to 85%
