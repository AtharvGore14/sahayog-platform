...
class RevenueEntry(Base):
    """Manual revenue entries linked to transactions - allows companies to record actual revenue received"""
    __tablename__ = "revenue_entries"
    
    id = Column(Integer, primary_key=True, index=True, autoincrement=True)
    transaction_id = Column(Integer, ForeignKey("waste_transactions.id"), nullable=False, index=True)
    company_id = Column(String, ForeignKey("companies.id"), nullable=False, index=True)
    
    # Revenue details
    actual_revenue = Column(Float, nullable=False)  # Actual revenue received
    expected_revenue = Column(Float, nullable=True)  # Expected revenue (from transaction calculation)
    revenue_variance = Column(Float, nullable=True)  # Difference between actual and expected
    
    # Additional metadata
    revenue_date = Column(DateTime, default=datetime.utcnow, nullable=False, index=True)
    payment_method = Column(String, nullable=True)  # "cash", "bank_transfer", "check", "digital", etc.
    vendor_name = Column(String, nullable=True)  # Who paid for the waste
    invoice_number = Column(String, nullable=True)  # Invoice reference
    notes = Column(Text, nullable=True)  # Additional notes
    
    # Status tracking
    status = Column(String, default="recorded")  # "recorded", "verified", "reconciled", "disputed"
    recorded_by = Column(String, nullable=True)  # User who recorded this revenue
    verified_by = Column(String, nullable=True)  # User who verified this revenue
    verified_at = Column(DateTime, nullable=True)
    
    # Relationships
    transaction = relationship("WasteTransaction", backref="revenue_entries")
    company = relationship("Company")

class CostEntry(Base):
    """Manual cost entries linked to transactions - allows companies to record actual costs incurred"""
    ...
    # Cost details
    actual_cost = Column(Float, nullable=False)  # Actual cost incurred
    expected_cost = Column(Float, nullable=True)  # Expected cost (from transaction calculation)
    cost_variance = Column(Float, nullable=True)  # Difference between actual and expected
    
    # Cost breakdown
    disposal_cost = Column(Float, nullable=True)  # Disposal/landfill cost
    transportation_cost = Column(Float, nullable=True)  # Transportation/logistics cost
    processing_cost = Column(Float, nullable=True)  # Processing/treatment cost
    other_costs = Column(Float, nullable=True)  # Other miscellaneous costs
    
    # Additional metadata
    cost_date = Column(DateTime, default=datetime.utcnow, nullable=False, index=True)
    cost_type = Column(String, nullable=True)  # "disposal", "transportation", "processing", "other", "mixed"
    vendor_name = Column(String, nullable=True)  # Vendor/service provider who charged the cost
    invoice_number = Column(String, nullable=True)  # Invoice/bill reference
    payment_method = Column(String, nullable=True)  # How the cost was paid
    notes = Column(Text, nullable=True)  # Additional notes
    
    # Status tracking
    status = Column(String, default="recorded")  # "recorded", "verified", "reconciled", "disputed"
    recorded_by = Column(String, nullable=True)  # User who recorded this cost
    verified_by = Column(String, nullable=True)  # User who verified this cost
    verified_at = Column(DateTime, nullable=True)
    
    # Relationships
    transaction = relationship("WasteTransaction", backref="cost_entries")
    company = relationship("Company")