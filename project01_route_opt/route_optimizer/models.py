...
class Vehicle(models.Model):
    ...
    capacity = models.DecimalField(
        max_digits=10, 
        decimal_places=2,
        help_text="Waste capacity in liters"
    )
    ...
    def calculate_fuel_required(self, distance_km: float) -> float:
        """Calculate fuel required for a given distance in km."""
        if self.fuel_efficiency and self.fuel_efficiency > 0:
            return round(float(distance_km / self.fuel_efficiency), 2)
        return 0.0
    
    def can_complete_route(self, distance_km: float):
        """
        Check if vehicle can complete route with current fuel tank capacity.
        Returns (can_complete, fuel_required)
        """
        fuel_required = self.calculate_fuel_required(distance_km)
        can_complete = fuel_required <= float(self.fuel_tank_capacity)
        return can_complete, fuel_required
