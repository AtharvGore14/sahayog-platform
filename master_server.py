...
- the FastAPI project contained in `project04` (wrapped by a tiny ASGI→WSGI
  adapter), and
- the Django marketplace project in `sahayog_marketplace`, proxied through
...
PROJECT03_PATH = os.path.join(BASE_DIR, 'sahayog_marketplace')
...
class SubprocessProxyApplication:
    ...
    def __call__(self, environ, start_response):
        ...
        # DispatcherMiddleware strips the route prefix before passing to the proxy:
        # - /api/marketplace/users/ -> proxy receives /users/ (SCRIPT_NAME might be /api/marketplace)
        # - /marketplace/file.html -> proxy receives /file.html (SCRIPT_NAME might be /marketplace)
        # - /api-token-auth/ -> proxy receives / (SCRIPT_NAME might be /api-token-auth)
        # 
        # We reconstruct the full path by checking SCRIPT_NAME or path patterns
        
        # Check SCRIPT_NAME first to detect which route this came from
        # DispatcherMiddleware sets SCRIPT_NAME to the matched prefix and strips it from PATH_INFO
        
        if path.startswith('/api/marketplace'):
            # Already has full path, forward as-is
            forward_path = path
        elif path.startswith('/api-token-auth'):
            # /api-token-auth/ is at Django root level, forward as-is
            forward_path = path
        elif script_name == '/api-token-auth':
            # DispatcherMiddleware routed /api-token-auth/ -> stripped to / or empty
            # Reconstruct the full path
            forward_path = '/api-token-auth/' if (path == '/' or path == '') else '/api-token-auth' + path
        elif self.mount_path and path.startswith(self.mount_path):
            # Strip /marketplace prefix for frontend files
            forward_path = path[len(self.mount_path):] or '/'
        elif path.startswith('/media/'):
            # Media files are served directly by Django at /media/
            # Forward to Django subprocess as-is
            forward_path = path
        elif script_name == '/media':
            # DispatcherMiddleware routed /media/... -> stripped to /... or /listings/...
            # Reconstruct the full path
            forward_path = '/media' + path if not path.startswith('/media') else path
        elif script_name == '/api/marketplace' or path.startswith(('/users/', '/listings/', '/bids/', '/commodities/', '/notifications/', '/ai/', '/me/', '/auth/')):
            # API endpoint paths under /api/marketplace/ that were stripped by DispatcherMiddleware
            # Prepend /api/marketplace to reconstruct the full path Django expects
            forward_path = '/api/marketplace' + path
        # else: forward_path remains as path (for root / or other paths)
...
# Note: /api/marketplace and /api-token-auth are routed to marketplace_proxy_application
application = DispatcherMiddleware(
    landing_application,
    {
        ...
        '/api/marketplace': marketplace_proxy_application,  # Route API calls to marketplace
        '/api-token-auth': marketplace_proxy_application,  # Route auth token endpoint to marketplace
        '/media': marketplace_proxy_application,  # Route media files to marketplace Django server
