...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = UserProfile
        fields = ('username', 'user_type', 'ai_trust_score', 'ai_success_rate', 'preferred_bid_strategy')
...
class ListingSerializer(serializers.ModelSerializer):
    ...
    image_url = serializers.SerializerMethodField()  # Add image_url field with full URL
        ...
        if obj.image:
            ...
            if request:
                ...
                if image_url.startswith('http'):
                    return image_url
                # Build absolute URI - use the request's scheme and host
                # This ensures the URL points to the master server (port 8000) not Django's internal port
                absolute_url = request.build_absolute_uri(image_url)
                # If the URL contains the Django internal port (9100), replace with master server port (8000)
                if ':9100' in absolute_url:
                    absolute_url = absolute_url.replace(':9100', ':8000')
                return absolute_url
            # Fallback: return relative URL (frontend will prepend API_BASE)
